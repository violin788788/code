...
"""
file_to_make_bat_of = file_to_make_bat_of.replace("\n","")
file_to_make_bat_of = file_to_make_bat_of.replace("","")

if "." not in file_to_make_bat_of:
    print('have to put "." and file type in file..')
    sys.exit()

dot = file_to_make_bat_of.find(".")
base = file_to_make_bat_of[0:dot]
bat_file = base+".bat"
...
with open(bat_file, 'w') as bat:
    bat.write(f'@echo off\n')
    bat.write(f'python {file_to_make_bat_of}\n')
    bat.write(f'pause\n')
print(f"Batch file '{bat_file}' created successfully to run {file_to_make_bat_of}")
