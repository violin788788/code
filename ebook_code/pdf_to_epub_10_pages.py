...
while(quit<1):
    count = count+1
    print(count,"of",len(pdf))
    try:
        page = pdf[count]
    except:
        break
    if count==0:
        chapter=epub.EpubHtml(title=f"Pages {count}-{count+10}",file_name=f"Pages {count}-{count+10}.xhtml",lang="en")
    page_text=page.get_text("text").strip()
    chap_text = chap_text+page_text
    if count>0:
        end = str(count)
        end = end[len(end)-1:len(end)]
        if end=="0":
            chapter.content=f"<html><body><h2>Pages {count}-{count+10}</h2><p>{chap_text}</p></body></html>"
...
for i,page in enumerate(pdf):
    ...
    more = more+text
    if i==0:
        chapter=epub.EpubHtml(title=f"Pages {i}-{i+10}",file_name=f"Pages {i}-{i+10}.xhtml",lang="en")
    chapter.content=f"<html><body><h2>Page {i+1}</h2><p>{more}</p></body></html>"
        continue
    end = str(i)
    end = end[len(end)-1:len(end)]
    if end=="0":
        ...
        chapter=epub.EpubHtml(title=f"Pages {i}-{i+10}",file_name=f"Pages {i}-{i+10}.xhtml",lang="en")
        chapter.content=f"<html><body><h2>Page {i+1}</h2><p>{chapter.content}</p></body></html>"
    print(i,page)
